package main.java.jforex.pyramid.version0_1;
 ...
 * Time: 2:45 PM
 * version 0.1
 * macd, tp i sl ustawia sie za pozno, trailling stop dziala jesli ustawiony > 0
 */
...
public class PyramidStrategy implements IStrategy {
	...
	private int transactionDifferenceOpen = 15;
	...
	protected void openTransaction() throws JFException {
		...
		if (openTrades == 0) {
			currentCommand = checkForOpen(instrument);
			double nextLot = getLots(instrument);
			if (currentCommand == IEngine.OrderCommand.BUY) {
				if (nextLot > 0.0) {
					IOrder order = openOrder(instrument, currentCommand, nextLot);
	...
	protected IEngine.OrderCommand checkForOpen(Instrument instrument) throws  JFException{

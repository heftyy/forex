package main.java.jforex.pyramid.version0_2;
 ...
 * Time: 2:38 PM
 * version 0.2
 * macd or sma, TPSL fixed, no trailing stop
 * macd lub sma, tp i sl ustawiaja sie tak ja powinny, nie ma trailing stopa
 */
...
public class PyramidStrategy implements IStrategy {
	...
	public int trailingStop = 0;
	//    @Configurable("Trailing stop diff[pips]")
	public int trailingStopDiff = 0;

	@Configurable("Next transaction open difference[pips]")
	public int transactionDifferenceOpen = 15;
	@Configurable("Next transaction close difference[pips]")
	private int transactionDifferenceClose = 120;
	...
	public PyramidStrategy() {
		loadConfig("pyramid_robot.properties");
	}
	...
	protected void setTakeProfit(Instrument instrument, double takeProfit) throws JFException {
		takeProfit = roundToPippette(takeProfit, instrument);
		for (IOrder order : engine.getOrders(instrument)) {
			if (order.getState() == IOrder.State.FILLED) {
	...
	protected void setStopLoss(Instrument instrument, double stopLoss) throws JFException {
		stopLoss = roundToPippette(stopLoss, instrument);
		for (IOrder order : engine.getOrders(instrument)) {
			if (order.getState() == IOrder.State.FILLED) {
	...
	protected IOrder openOrder(Instrument instrument, IEngine.OrderCommand command, double lot) throws JFException {
		...
		if(command == IEngine.OrderCommand.BUY) {
			order = engine.submitOrder(getLabel(instrument), instrument, IEngine.OrderCommand.BUY, lot, 0, 5);
		} else if(command == IEngine.OrderCommand.SELL) {
			order = engine.submitOrder(getLabel(instrument), instrument, IEngine.OrderCommand.SELL, lot, 0, 5);
		}
		if ( order != null ) {
		}
		return order;
	...
	public void onMessage(IMessage message) throws JFException {
		...
		if(message.getType() == IMessage.Type.ORDER_FILL_OK) {
			double[] tpsl = calculateTPSL(currentCommand);
			double tp = roundToPippette(tpsl[0], instrument);
			double sl = roundToPippette(tpsl[1], instrument);
			setTakeProfit(instrument, tp);
			setStopLoss(instrument, sl);
		}
	}
	...
	public void loadConfig(String name) {
		...
		System.out.println("Found config file, reading now");
		maxTrades = Integer.parseInt(prop.getProperty("max_trades"));
		lots = Float.parseFloat(prop.getProperty("lot"));
		multiplier = Float.parseFloat(prop.getProperty("multiplier"));
		maPeriod = Integer.parseInt(prop.getProperty("ma_period"));
		maShift = Integer.parseInt(prop.getProperty("ma_shift"));
		takeProfitPips = Integer.parseInt(prop.getProperty("tp_pips"));
		stopLossPips = Integer.parseInt(prop.getProperty("sl_pips"));
		String periodString = prop.getProperty("period");
		if(period.equals("1_HOUR")) {
			period = Period.ONE_HOUR;
		} else if(periodString.equals("5_MIN")) {
			period = Period.FIVE_MINS;
		}
		transactionDifferenceOpen = Integer.parseInt(prop.getProperty("pips_next_open"));
		transactionDifferenceClose = Integer.parseInt(prop.getProperty("pips_next_close"));
		macdFast = Integer.parseInt(prop.getProperty("macd_fast"));
		macdSlow = Integer.parseInt(prop.getProperty("macd_slow"));
		macdSignal = Integer.parseInt(prop.getProperty("macd_signal"));
		macdShift = Integer.parseInt(prop.getProperty("macd_shift"));
		macdHistTest = Float.parseFloat(prop.getProperty("macd_hist"));
	}
}
